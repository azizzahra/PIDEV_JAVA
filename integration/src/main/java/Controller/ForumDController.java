...
public class ForumDController {
    ...
    public void initialize() {
        ...
        categories.addAll("Toutes les catégories", "Agrotech", "matériel agricole", "Energies vertes");
        ...
        sortingOptions.addAll("Plus récents d'abord", "Plus anciens d'abord", "Plus de vues", "Plus de votes");
        sortOptions.setItems(sortingOptions);
        sortOptions.setValue("Plus récents d'abord");
        sortOptions.setOnAction(event -> loadPosts());
    ...
    private void loadPosts() {
        ...
        List<post> filteredPosts = posts.stream()
                ...
                .filter(p -> searchTerm.isEmpty() ||
                        p.getTitle().toLowerCase().contains(searchTerm) ||
        ...
        switch (sortOption) {
            case "Plus récents d'abord":
                filteredPosts.sort((p1, p2) -> comparePostDates(p2.getDateC(), p1.getDateC())); // Newest first
                break;
            case "Plus anciens d'abord":
                filteredPosts.sort((p1, p2) -> comparePostDates(p1.getDateC(), p2.getDateC())); // Oldest first
                break;
            case "Plus de vues":
                filteredPosts.sort((p1, p2) -> Integer.compare(p2.getViews(), p1.getViews())); // Most views first
                break;
            case "Plus de votes":
                filteredPosts.sort((p1, p2) -> Integer.compare(p2.getVote(), p1.getVote())); // Most votes first
                break;
        }
    ...
    private int comparePostDates(String date1, String date2) {
        try {
    ...
    @FXML
    private void showForumStatistics() {
        Stage statisticsStage = new Stage();
        statisticsStage.setTitle("Statistiques du Forum AgroSmart");

        TabPane tabPane = new TabPane();

        // Tab 1: Category Statistics
        Tab categoryTab = new Tab("Statistiques par catégorie");
        categoryTab.setClosable(false);

        PieChart categoryChart = createCategoryPieChart();
        VBox categoryVBox = new VBox(20, categoryChart);
        categoryVBox.setPadding(new Insets(20));
        categoryTab.setContent(categoryVBox);

        // Tab 2: Engagement Statistics
        Tab engagementTab = new Tab("Statistiques d'engagement");
        engagementTab.setClosable(false);

        BarChart<String, Number> engagementChart = createEngagementBarChart();
        VBox engagementVBox = new VBox(20, engagementChart);
        engagementVBox.setPadding(new Insets(20));
        engagementTab.setContent(engagementVBox);

        // Add tabs to tabPane
        tabPane.getTabs().addAll(categoryTab, engagementTab);
    ...
    private PieChart createCategoryPieChart() {
        ...
        chart.setTitle("Répartition des posts par catégorie");
        chart.setLabelsVisible(true);

        return chart;
    }

    private BarChart<String, Number> createEngagementBarChart() {
        List<post> allPosts = postService.afficher();

        // Calculate average views and votes per category
        Map<String, Double> avgViewsByCategory = allPosts.stream()
                .collect(Collectors.groupingBy(
                        post::getCategory,
                        Collectors.averagingInt(post::getViews)
                ));

        Map<String, Double> avgVotesByCategory = allPosts.stream()
                .collect(Collectors.groupingBy(
                        post::getCategory,
                        Collectors.averagingInt(post::getVote)
                ));
        ...
        barChart.setTitle("Engagement moyen par catégorie");

        // Create series for views
        XYChart.Series<String, Number> viewsSeries = new XYChart.Series<>();
        viewsSeries.setName("Vues moyennes");

        // Create series for votes
        XYChart.Series<String, Number> votesSeries = new XYChart.Series<>();
        votesSeries.setName("Votes moyens");

        // Add data to series
        for (String category : avgViewsByCategory.keySet()) {
            viewsSeries.getData().add(new XYChart.Data<>(category, avgViewsByCategory.get(category)));
            votesSeries.getData().add(new XYChart.Data<>(category, avgVotesByCategory.get(category)));
        }

        barChart.getData().addAll(viewsSeries, votesSeries);
        return barChart;
    }
    ...
    private void handleReturnToDashboard() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/Post/dashboard.fxml"));
